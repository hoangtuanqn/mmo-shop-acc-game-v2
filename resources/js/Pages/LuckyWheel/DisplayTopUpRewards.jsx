    ...
    return (
        <>
            {topUpRewards.length <= 0 ? (
                <>
                    <p className="mt-6 text-center text-base font-bold">Hiện tại chưa có phần quà</p>
                </>
            ) : (
                <ul className="pt-4">
                    {topUpRewards.map((item, key) => (
                        <li className="rounded-sm px-2 py-2 even:bg-[#F8F8FC]" key={`${item.balance}-${item.id}`}>
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-primary flex items-center rounded-sm px-5 py-1 text-xs font-semibold xl:text-sm">
